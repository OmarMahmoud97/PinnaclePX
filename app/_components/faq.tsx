        ...
        <div className="divide-y divide-border p-8 md:col-span-4 md:p-12">
          {FAQ_ITEMS.map(({ question, answer }) => (
            <details key={question} className="group py-4 first:pt-0 last:pb-0">
              <summary className="flex cursor-pointer list-none items-start justify-between gap-4 rounded-md font-medium outline-none focus-visible:ring-[3px] focus-visible:ring-brand/50 [&::-webkit-details-marker]:hidden">
                {question}
                <ChevronDown
                  aria-hidden="true"
