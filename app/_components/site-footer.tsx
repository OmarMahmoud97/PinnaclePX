...
// assistive technology and cannot be selected, and it stays inside the clip of the box the
// content sits in, so a phone never scrolls sideways to reach the rest of it. From md up the
// choreography (app/_components/motion/footer.ts) scrubs it the last of the way up as the sheet
// arrives. The clip is on that box and not on the sheet because the sheet's top is liquid from
// md up (app/_components/motion/sheet-lip.ts, ADR 0034): the empty svg before the box is the
// lip, which the choreography draws as the same rounded corners in the sheet's own pixels and
// swells above the sheet with the scroll; until then, and on phones and under reduced motion,
// it is not shown and the CSS corners stand. The box is positioned so it paints over the lip,
// whose foot reaches under the column headings: a positioned element paints over in-flow text
// whatever the DOM order, and the lip is one.
export function SiteFooter() {
  return (
    <footer data-theme="dark" className="sheet bg-surface">
      <svg className="sheet-lip" preserveAspectRatio="none" aria-hidden="true" focusable="false">
        <path />
      </svg>
      <div className="relative overflow-clip pt-band-sm">
        <div className={shell}>
