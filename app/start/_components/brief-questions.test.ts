...
describe('QUESTIONS', () => {
  it('uses the company name once it is known', () => {
    const answers = { ...INITIAL_ANSWERS, company: ' Ashgrove Physio ' }
  ...
  it('falls back to "your" before a company name exists', () => {
    expect(QUESTIONS.logo.title(INITIAL_ANSWERS)).toBe('Add your logo, or skip it.')
    expect(QUESTIONS.colours.title(INITIAL_ANSWERS)).toBe('Pick your colours.')
  })
})
