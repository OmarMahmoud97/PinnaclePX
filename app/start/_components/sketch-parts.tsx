...
// two sizes in a table so a change to the part reaches both frames.
...
export function Bar({ className }: { className: string }) {
  return <span className={cn('block rounded-full bg-(--sketch-line)', className)} />
}
...
export function Wordmark({ model, frame }: FrameProps) {
  ...
  return (
    <span key={model.logo ?? model.company} className={cn('sketch-in flex items-center', size.row)}>
...
export function Headline({ model, frame }: FrameProps) {
  ...
  if (model.company === '') return <Slot label="your company" className={size.slot} />
  return (
    <p
      className={cn('sketch-in leading-tight font-semibold tracking-tight text-balance', size.text)}
    >
      {model.company}
    </p>
  )
...
export function Paragraph({ model, frame }: FrameProps) {
  ...
  if (model.description === '') return <Slot label="your words" className={size.slot} />
  return (
    <p
      className={cn(
        'sketch-in leading-snug transition-colors duration-400',
...
export function ImageBlock({ model, className }: { model: SketchModel; className: string }) {
  ...
  return (
    <div
      ...
      className={cn(
        'sketch-in relative overflow-hidden rounded-lg border border-(--sketch-line) transition-colors duration-400',
        fill,
        className,
      )}
    >
      ...
      {model.imageLabel === null ? (
        ...
        <span className="absolute bottom-1.5 left-2 max-w-[90%] truncate rounded-full bg-(--sketch-bg) px-1.5 py-0.5 font-mono text-[8px] text-(--sketch-muted)">
          {model.imageLabel}
        </span>
      )}
    </div>
