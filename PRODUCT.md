...
- Not built yet: nine of the ten templates (placeholders; `t01-aurora` is built and viewable at `/examples/aurora`, ADR 0008), the examples gallery, rate limits, the honeypot, the privacy notice, the retention job and erasure. Before traffic: a verified Resend sending domain (`RESEND_FROM`), the Anthropic key's workspace id (`ANTHROPIC_WORKSPACE_ID`) or a workspace-scoped key, and the Vercel project's environment variables.
