...
- Built: the deterministic pipeline (ADR 0009). Submitting stores the lead and the submission, chooses the templates the visitor has not seen, derives the colour tokens, reads the logo, and renders a design at `/preview/[slug]/[templateId]` within seconds. Copy is the deterministic fallback in the visitor's own words until the model stages land; imagery is not yet placed.
- Not built yet: the model stages (brief and copy), the imagery stage (own photos and Pexels), the email with the link, nine of the ten templates (placeholders; `t01-aurora` is built and viewable at `/examples/aurora`, ADR 0008), the examples gallery, rate limits and the privacy notice. No traffic may be sent to `/start` until those exist.
