...
**D8. The phone window is the phone frame.** Below `lg`, in a region under 36rem wide, the draft's phone frame is restyled as a 1:1 window, 358 by 212: one element, `data-frame="phone"`. From 36rem to `lg` the region shows a 300 px crop of the desk layout. Under 47.5rem tall either crop is 150 px; under 30rem tall the window and curve hide, so the h1 is in the first screen.
...
| **768 by 1024**                                                                 | Stacked; the region shows a 704 by 300 crop of the draft's desk layout (container ≥ 36rem), about 396 px plus the curve; card `max-w-xl`; H1 48 px                                                                                                                            |
| **Split views** (507, 678 wide); **700 by 500**                                 | Stacked. 507: window `min(100cqi - 32px, 480px)`, centred. 678: the 768 crop. 700 by 500: the 150 px crop; h1 in the first screen                                                                                                                                             |
| **390 by 844**                                                                  | Region 308 px: island clearance, the window (the phone frame restyled, 358 by 212, bar "Live draft"), the curve. H1 36 px at y about 387. Below `sm` the card is flat (controls and white wells on the wash). Sticky ask; from q2 a 48 px round Back beside it                |
| **390 by 664** (under 47.5rem tall)                                             | Crop 150 px, region about 226, H1 at y about 306; short hint; look tiles as 64 px rows. Estimated gaps from the first control to the ask: q1 36, q2 50, q3 60, q4 90, q5 60 (≥ 24 required)                                                                                   |
| **320 by 640**                                                                  | Window 288 wide; look tiles one column; `scrollWidth` 320; below 22.5rem wide Back sits under a full-width ask; done posters 84 by 115                                                                                                                                        |
| **Any width under 30rem tall** (844 by 390, 932 by 430, 667 by 375, 320 by 256) | Window and curve hidden; the region keeps only the island's clearance and the screen-reader sentence; the ask is static; h1 and first control in the first screen                                                                                                             |
...
| Window offset (phone)                      | Question change                   | opacity out, the offset jumps unseen, opacity in                                                                                                                                                      | 450 ms                                                 | 0                      | CSS                                                                                                         | Instant jump                                  |
...
| Phone region at 390 by 844              | 309 of 320                                      | Window 212                                                                                                                                                                                                                                                                                                                                                           | 308                                                                                 |
| First control to the ask at 390 by 664  | q1 48, q3 46, q4 83, q5 83                      | Shorter hints; hand-off receipts only (D6)                                                                                                                                                                                                                                                                                                                           | ≥ 24 required, about 36 expected                                                    |
...
| `data-frame="browser"` and `"phone"`; the window is the phone frame, first in the DOM below `lg`                                                                           | P6                                                                | `brief-shell.spec.ts`                                                                       |
