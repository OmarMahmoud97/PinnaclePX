...
| 11  | `CornerTicks` at the stage                                                   | removed from the home usage; the component stays for `/start`                                            |
...
- 390: Amended 23 September 2026 by the director (A2): ONE column (`grid-cols-1`), `gap-3`, tiles `rounded-2xl p-4`, phones `max-w-48` (192 px, under the 224 px ceiling), the control above the frame, captions `text-label`, the button `w-full`. The two-up phone layout is withdrawn; the phone soft line (9.4) is reported either way.
...
**Acceptance.** Screenshot at 1440: no visible edge between the hero's foot and the band; six dark tiles with a soft cyan pool behind each phone; the checked pill is light on dark; the result lines are cyan; the button is navy with white text on the band, not on a tile. At 390: one column (Amended 23 September 2026 by the director, A2), phones 192 px wide, no text wall (trade, did, result and the link each on their own lines with room). Tests: `home.spec` Work block, `mobile.spec` `scrollWidth === 390`, axe zero violations with the `target-size` of the 28 px pills.
...
- `CornerTicks` is removed from this file (the component stays for `/start`).
