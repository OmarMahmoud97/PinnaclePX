...
| `lib/ai/brief.ts`                                                           | `writeBrief(answers)`: `messages.parse` with `zodOutputFormat(briefSchema)`, `claude-sonnet-5`, adaptive thinking, effort `medium`. Paraphrase only; invent no numbers, awards, clients or claims.                                                                                                                                                                  | IO            |
...
| `app/api/upload/route.ts`                                                   | `handleUpload` from `@vercel/blob/client`. Token issued only for pathnames matching `^(logos\|photos)/[a-f0-9]{64}\.(png\|jpg\|jpeg\|webp\|svg)$`, allowed content types, `maximumSizeInBytes: CONFIG.form.maxUploadBytes`, `addRandomSuffix: false`, `allowOverwrite: true`. `onUploadCompleted` is not used: it cannot reach localhost and nothing depends on it. | IO            |
...
| ----- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | --------------------------------- |
| brief | `claude-sonnet-5`  | A stable system prompt (cached with `cache_control`), then the company, the sentence and the style.                                                                                     | `BrandBrief`                                          | 20 s, effort `medium`             |
| copy  | `claude-sonnet-5`  | The same system prompt, the brief, the template's slot table (path, min, max, one line on what the slot is for), the rules. Retry message: the first output and the list of violations. | the template's `copySchema`                           | 60 s per attempt, effort `medium` |
