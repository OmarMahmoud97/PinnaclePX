...
Section 7 is amended by ADR 0037 (the first release of `docs/start-page-journey-plan.md`, 24 September 2026). The URL gains `?q=done&s={slug}`, so a refresh or a pasted link reopens the done view. The flow moves by `pushState` and `replaceState` rather than `router.push` and `router.back()`, and one Back from done leaves `/start`. The guard stops at the lower of the furthest question shown (`reached`, kept in the draft) and the first invalid one. A send leaves keys that bring the done view back (`app/start/_components/done-storage.ts`), and the done view is a chunk of its own. `initialsFrom` is gone, and `brief_step` and `brief_error` carry a question id rather than a step. The later releases amend the rest.

Its second release (25 September 2026) supersedes section 4's order, headings and helpers: the questions run sentence, name, look, colour, send (`lib/brief/question-ids.ts`), under fixed headings, and every word they say lives in `app/start/_components/start-copy.ts`. In section 7, `brief-questions.ts` and `logo-step.tsx` are gone, `brand-step.tsx` asks for the business name and its mark, each question sets the tab's title, and `app/start/page.tsx` imports its own stylesheet, `app/_styles/start.css`, which `app/globals.css` no longer carries.

Its third release (25 September 2026) replaces section 7's `brief-sketch.tsx` with the live draft in `app/start/_components/draft/`, and `sketch-chips.tsx` keeps only the sentence a screen reader hears. The steps after the first load as one chunk while the first question is answered (`steps/later-steps.tsx`). The done view draws the designs with `design-list.tsx`, the stages with `stage-ring.tsx` and `done-log.tsx`, and the send with `send-bloom.tsx`, where the countdown and the design slots were. `app/start/page.tsx` imports two more sheets of its own, `start-draft.css` and `start-done.css`.
