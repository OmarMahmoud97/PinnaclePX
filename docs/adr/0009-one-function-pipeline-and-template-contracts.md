...
- Built on 4 September 2026 as the deterministic loop (plan slice 3): `lib/inngest/functions/build-concepts.ts` and `sweep-deadline.ts`, `lib/db/exclusivity.ts` and `submissions.ts`, `lib/select`, `lib/identity`, `lib/logo`, `lib/preview/status.ts`, and the preview routes. Until the model stages land, the brief and copy stages are marked `fallback`, because that is what they are, and the imagery stage settles with no pictures. Locally the Inngest SDK reads `INNGEST_DEV=1` and sends to the dev server; the e2e test that submits runs only where `E2E_SUBMIT=1` (Playwright `metadata.canSubmit`), since it needs the database and the dev server. Verified: five answers to a linked Aurora page in under four seconds; the same answers return the same slug; a second submission from the same address is shown the book-a-call state; the rendered page is axe clean at 1440 and 390 px.
