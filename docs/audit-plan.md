...
- Progress: every item was done on 4 September 2026 and is recorded in ADR 0017 (items 1 to 5), ADR 0018 (items 6 to 12) and ADR 0019 (items 13 to 18); see the Progress section. The findings below are kept as they were written, so the line numbers refer to commit `62d409d`, not to the code after the changes.
...
| 13   | 1.2 zod 4 and the other majors           | Done, two majors held back    | ADR 0019, decisions 1 and 7 | zod 4.5.4 everywhere, one import path, v3-only calls moved; the standards pin updated. vitest 5.0.0 passes the suite unchanged (`engines.node` is now `>=22.12`); `@types/node` pinned to 22.x. ESLint 10 crashed in `eslint-plugin-react` 7.37.5, which supports up to 9.7, so ESLint stays at 9; TypeScript 7 is outside `typescript-eslint`'s range (`<6.1.0`), so TypeScript stays at 5.9. |
| 14   | 1.7 upload hook out of `Flow`            | Done                          | ADR 0019, decision 5        | `usePictureUploads(answers, dispatch)` holds the object URLs, the outcomes and the handlers; the desktop brief e2e spec passed against it.                                                                                                                                                                                                                                                     |
| 15   | 1.3 rank prompt into `prompts.ts`        | Done                          | ADR 0019, decision 2        | `RANK_SYSTEM_PROMPT` and `rankPrompt(purpose)`; `rank.ts` throws `AppError`.                                                                                                                                                                                                                                                                                                                   |
| 16   | 4.2 compact JSON in the copy prompt      | Done                          | ADR 0019, decision 3        | `JSON.stringify(brief)`.                                                                                                                                                                                                                                                                                                                                                                       |
| 17   | 2.2 SVG hardening                        | Done, guard rather than trust | ADR 0019, decision 4        | The sharp documentation says nothing about the bundled librsvg's policy, so `lib/logo/svg.ts` refuses scripts, external or file hrefs, `javascript:` URLs, entity declarations, external doctypes and XIncludes before the rasteriser.                                                                                                                                                         |
| 18   | 5.4 blob-reference table                 | Done                          | ADR 0019, decision 6        | `blob_ref(url, slug)` written on submit and by `markStage`; `urlReferencedElsewhere` is one indexed read. Migration `0005` creates and backfills it; applied to the development database and checked row by row: 17 submissions, 45 references, no mismatches.                                                                                                                                 |

Changes made alongside, not named in the findings: a stage's work now returns how it ended (`Written`, with `done(patch)` for the common case), so a stage can settle itself with its own fallback; `imageryFor` takes every chosen template at once and returns which slots a retryable failure left empty; `rehostImage` takes bytes; `lib/download.ts`, `lib/blob/read-upload.ts`, `lib/ai/errors.ts`, `lib/preview/read.ts` and `lib/inngest/remove-submission.ts` are new. Checks after all three batches: lint, typecheck, knip, prettier, build and bundle budget clean (the start page budget raised to 245 KB for zod 4, ADR 0019); 40 test files, 379 tests; `pnpm audit` clean; the desktop brief e2e spec passing.
...
### Nice to have (done, ADR 0019)

13. **zod 4 with the other majors** (1.2, section 6). Hours. One PR per major; zod first because it simplifies code, TypeScript 7 last. Done for zod 4, vitest 5 and the Node 22 types; ESLint 10 and TypeScript 7 wait for their plugins.
14. **Extract the upload hook from `Flow`** (1.7). Hours. Done.
15. **Move the rank prompt into `prompts.ts`** (1.3). Minutes. Done.
16. **Compact JSON in the copy prompt** (4.2). Minutes. Done.
17. **SVG hardening** (2.2), once the librsvg policy is confirmed. Under an hour. Done; the policy is undocumented, so the guard stands on its own.
18. **A blob-reference table for retention** (5.4), when submissions reach the thousands. Hours. Done, with a backfill.
