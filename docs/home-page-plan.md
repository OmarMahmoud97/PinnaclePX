...
| `lib/site.ts`                                                      | Replace the placeholder `bookingUrl` (`https://cal.com/pinnaclepx`). It is live in the hero, taster, closing section and footer; a dead booking link is a Stanford "error of any size" on the one action that makes money. Add `town` and `contactEmail` when supplied.             | owner                                    |
